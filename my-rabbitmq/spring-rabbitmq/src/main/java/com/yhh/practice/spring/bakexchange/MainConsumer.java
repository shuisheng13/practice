...
public class MainConsumer {
    ...
    public static void main(String[] args) {
        ...
        try{
            ...
            //信道绑定到交换器
            channel.exchangeDeclare(Constant.MAIN_EXCHANGE_NAME, BuiltinExchangeType.DIRECT);
            //获取随机队列
            //String queueName = channel.queueDeclare().getQueue();
            String queueName = Constant.QUEUE_MAIN;
            //所有日志严重性级别  路由
            String[] severities={"info"};
            for (String severity : severities) {
                //信道绑定队列绑定交换器绑定路由
                channel.queueBind(queueName,Constant.MAIN_EXCHANGE_NAME,severity);
            }
            ...
            final Consumer consunmer1 = new DefaultConsumer(channel) {
                ...
                public void handleDelivery(String consumerTag, Envelope envelope,
                    ...
                    System.out.println("消费者收到消息 message "+"["+message+"]+  路由 "+"["+envelope.getRoutingKey()+"]"
                            +" 交换器 ["+envelope.getExchange()+"] 发送次数 ["+atomicInteger.incrementAndGet()+"]");
                }
            };
            //信道绑定队列和消费者
